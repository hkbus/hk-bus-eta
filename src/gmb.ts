...
export default function fetchEtas ({gtfsId, stopId, bound, seq}: fetchEtasProps): Promise<Eta[]> {
  return (
    ...
    .then(({data}) => (
      data
        .filter(({route_seq}: any) => ( bound === 'O' && route_seq === 1 ) || ( bound === 'I' && route_seq === 2 ) )
        .filter(({stop_seq}: any) => stop_seq === seq + 1)
        .reduce( (acc: Eta[], {eta}: any) => [
          ...acc, 
          ...eta.map( (data: any) => ({
            eta: data.timestamp,
            remark: {
