...
export const getUpcomingFerry = ({
  ...
  const getEta = (serviceFreq: Record<string, any>, date: Date): string[] => (
    Object.keys(serviceFreq).sort((a, b) => a < b ? -1 : 1).reduce((acc, tp) => {
      ...
      if ( ferryTime >= date ) {
        acc.push(
          ...
          String(ferryTime.getUTCDate()).padStart(2, "0") + "T" +
          String(ferryTime.getUTCHours()).padStart(2, "0") + ":" +
          String(ferryTime.getUTCMinutes()).padStart(2, "0") + ":00" +
          ".000+08:00"
        )
      }
      return acc
  ...
  const ret = Array(24).fill(0).reduce((acc, _, idx) => {
